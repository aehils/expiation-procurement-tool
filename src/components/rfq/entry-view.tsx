...
export function EntryView({ draftId, rfqNumber }: EntryViewProps) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-6">
      <div className="flex justify-between items-start mb-6">
        <div>
          <div className="flex items-center gap-2.5">
            ...
            <span className="px-2 py-0.5 text-[10px] font-medium bg-slate-200 text-slate-600 rounded uppercase tracking-wide">
              Draft
            </span>
          </div>
          <p className="text-slate-600 text-sm mt-0.5">#{rfqNumber}</p>
        </div>
        <div className="relative" ref={menuRef}>
          <button
            ...
            className="w-9 h-9 flex items-center justify-center rounded hover:bg-slate-200 text-slate-900 text-2xl leading-none font-black"
          >
            ⋮
          </button>
          {menuOpen && (
            <div className="absolute right-0 mt-2 w-44 bg-white border border-slate-200 rounded-md shadow-lg z-10 py-1">
              <button
                type="button"
                onClick={() => {
                  setMenuOpen(false);
      ...
      <div className="bg-white rounded-md shadow-xl p-4 border border-slate-100 mb-6">
        <Label htmlFor="requester" className="mb-1.5 block text-xs">
          Requester *
        </Label>
        <Input
          id="requester"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        {/* Form */}
        <div className="lg:col-span-7 bg-white rounded-md shadow-xl p-5 border border-slate-100">
          <form onSubmit={handleAdd} className="space-y-5">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <Label className="mb-1.5 block text-xs">Item Category *</Label>
                <Select
              ...
              <div>
                <Label className="mb-1.5 block text-xs">Request Quantity *</Label>
                <Input
                  type="number"
                  min="1"
                  value={quantityRaw}
            ...
            <div>
              <Label className="mb-1.5 block text-xs">Item Name *</Label>
              <Input
                value={form.itemName}
            ...
            <div>
              <Label className="mb-1.5 block text-xs">Specification</Label>
              <Textarea
                rows={2}
            ...
            <div>
              <Label className="mb-1.5 block text-xs">Item Description</Label>
              <Textarea
                rows={2}
        ...
        <div className="lg:col-span-5">
          <div className="bg-white rounded-md shadow-xl border border-slate-100 h-full flex flex-col">
            <div className="px-5 pt-5 pb-3 flex items-center justify-between">
              <h3 className="font-semibold text-base text-slate-800">Added</h3>
            ...
            <div className="flex-1 px-5 pb-4 overflow-auto">
              {items.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center text-slate-400">
                  ...
                  <p className="mt-1 text-sm">Add Item to RFQ</p>
                </div>
              ) : (
                <div className="space-y-3">
                  {items.map((item) => (
                    <div
                      key={item.tempId}
                      className="bg-slate-50 border border-slate-200 rounded p-3 hover:shadow-md transition-shadow"
                    >
                      ...
                      <div className="mt-2 pt-1.5 border-t border-slate-200 flex justify-end gap-3">
                        <button
                          type="button"
